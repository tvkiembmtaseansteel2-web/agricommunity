...
| 28/08/2026 | DeepSeek Harness | **Phase 10 — PoC KB từ video YouTube**: Edge Function `kb-from-video` (whitelist `video_sources` credibility≥2, AI tóm tắt, đối chiếu chéo KB, chỉ ghi `raw_articles` draft). Migration `20260906_phase10_video_kb.sql`. Verify HTTP 200, draft_id=10, credibility=2, conflict=false. *(Cần thêm kênh uy tín thật + pgvector.)* |
