...
- [x] **Đưa site production lên Public & xác minh hoạt động**: site bị **"Team protection"** (Project visibility = Private) mặc định khi tạo qua CLI → chuyển **Production visibility → Public** trong Netlify (Site config → General → Visitor access → Edit visibility). **Xác minh trên production**: đăng nhập thật `0901234567` ✅; trang home tải đủ **thời tiết thật** (28°C, mưa phùn, icon SVG mưa, khuyến nghị nông vụ), **badge "VAI TRÒ: NÔNG DÂN"** (phân cấp vai trò), **"Hôm nay vườn cần làm gì?"** + nhật ký (đã theo dõi 3 hoạt động) → **env vars Supabase hoạt động đúng**. `https://agricommunity.netlify.app` public, đăng nhập được.
- [x] **Cấu hình Supabase Auth Allowed Redirect URLs** (qua Management API `PATCH /config/auth`): **`site_url`** = `https://agricommunity.netlify.app`; **`uri_allow_list`** = `https://agricommunity.netlify.app/**, https://agricommunity.netlify.app`. Xác minh đọc lại config OK (disable_signup=false, mailer_autoconfirm=true).
- [x] **Bật CI GitHub Actions** (đã sửa 3 lỗi để chạy xanh): (1) `npm ci` → `npm install` (tránh `EBADPLATFORM` trên optional deps esbuild); (2) `node-version` 20 → **22** (vitest4/jsdom30 cần `webidl.util.markAsUncloneable` ≥22); (3) E2E cập nhật: nút **'Chất cấm'** nằm trong tab **Tôi** + hiệu đề **'Tham khảo chất cấm & hạn chế'** (đổi từ 'Xuất khẩu'/'MRL Advisor' theo UX Blueprint); đồng bộ `package-lock.json` (thêm `esbuild@0.28.2`). **CI chạy xanh: 8/8 bước success** (install + Vitest 68 + build + E2E 2).
- [x] **Mốc thời gian nhật ký chuẩn** (hiển thị + đưa vào prompt AI): `daysSince` chuẩn hoá về **đầu ngày địa phương** (tránh lệch ±1 ngày do UTC); hiển thị "📅 ngày • X ngày trước / hôm nay"; thông báo cảnh báo kèm ngày cụ thể ("(lần cuối 13/08)"); `formatGardenLogs` thêm "(X ngày trước/hôm nay)" vào prompt Bác sĩ AI → phân tích đúng mốc.
- [x] **Thời tiết theo từng vườn + trang chi tiết** (khi 1 người có nhiều vườn ở khu vực khác): mỗi vườn **lưu vị trí riêng** (đã có `lat/lng` qua GardenMap) → `fetchGardenWeather`/`fetchAllGardenWeather` lấy thời tiết Open-Meteo theo đúng tọa độ vườn; card "Hiểu vườn" hiện **thời tiết riêng từng vườn** + **bấm mở `GardenDetail`** (thời tiết khu vực vườn + khu A/B/C/D + nhật ký riêng vườn với "X ngày trước"); nút "Xem chi tiết". Giữ nguyên khung "Thời tiết nông vụ hôm nay" toàn cục theo GPS thật của người dùng.
- [x] **Thu gọn trang chủ** (XP gọn, nông dân thấy ngay việc cần làm): card **"Hôm nay vườn cần làm gì?"** giờ thêm **danh sách công việc theo TỪNG vườn** (chỉ vườn có status warn/risk, bấm mở chi tiết; nếu tất cả ổn → "✅ Các vườn đều ổn"); card **"Hiểu vườn"** thu gọn thành **danh sách vườn dạng dòng** (tên + thời tiết khu vực vườn + trạng thái + việc cần làm + nút "👁️ Xem chi tiết"/"+ Ghi nhật ký"/"🔍 Bác sĩ AI") — thay bản mở rộng chi tiết trước đây (đã chuyển vào `GardenDetail`). Trang chủ nhẹ, tập trung danh sách vườn.
- [x] **Việc cần làm theo vườn + nhắc lịch** (`computeTodayTasks` trong `gardenHealth.js`): tách từ findings của mỗi vườn thành **danh sách task** có loại (`water`/`fert`/`spray`/`issue`), mức ưu tiên (risk→cao, warn→trung bình), gợi ý thời điểm ("Hôm nay"/"Trong 1–2 ngày tới"), bỏ các ghi chú "good". Dùng ở: **"Hôm nay"** (mỗi vườn hiện nhiều task thay vì 1 dòng) và **`GardenDetail`** (card **"✅ Việc cần làm hôm nay"** với icon + mức ưu tiên + ⏳ thời điểm; nếu không có việc → "Vườn này không có việc gì cần ưu tiên"). Test `computeTodayTasks` +3 (bỏ good, vườn ổn→rỗng, risk trước warn).
- [x] **Bỏ "Nhật ký chăm sóc gần đây" + thiết kế lại "Tổng hợp sản lượng"** (Home): loại bỏ card nhật ký gần đây (nhật ký đầy đủ đã ở tab Nhật ký); card **"Tổng hợp sản lượng thu hoạch"** giờ hiển thị **tổng cả các vườn** (theo loại cây: "🌳 Sầu riêng (tổng) N kg", "☕ Cà phê (tổng) N kg") + **📍 Theo từng vườn** (mỗi vườn: tên + tổng kg + phân loại theo cây; chưa có dữ liệu → "Chưa có dữ liệu thu hoạch"). Dữ liệu `yields.garden_id` (đã lưu khi thêm thu hoạch) → cộng đúng vào từng vườn. Build + 72/72 test + E2E 2/2.
- [x] **Soft-delete (chống mất dữ liệu) — Phase 8**: thêm `deleted_at` cho `gardens`/`logs`/`yields`; RLS SELECT `AND deleted_at IS NULL` (row đã xóa tự ẩn); partial index `idx_*_active`; hàm `public.soft_delete(table, id, owner)` (kiểm tra ownership); `GardensManager.handleDelete` đổi `.delete()` → `.update({deleted_at})` (thông báo khôi phục được). Migration `20260904_phase8_soft_delete.sql` (đã `db push`). Đã cập nhật `supabase_schema.sql`.
- [x] **Lộ trình bảo vệ dữ liệu & mở rộng** (`docs/SCALING_ROADMAP.md` — bộ nhớ bền vững): ghi rõ **không tự xóa data**, RLS per-user, soft-delete đã có; kế hoạch theo giai đoạn 0–100 / 100–1000 / >1000 user (backup PG dump, export CSV, lên Pro lấy PITR, rate-limit+cache Gemini, replica); ước tính chi phí Netlify≈0đ, Supabase, Gemini; cách khôi phục row đã soft-delete.
- [x] **Blocker #1 — Gemini key chuyển sang server** (Supabase Edge Function `gemini-proxy`): tạo function nhận `{contents, generationConfig}` → gọi Gemini với `GEMINI_API_KEY` (secret) server-side, `verify_jwt=true` (chỉ user đã đăng nhập). Đổi 3 client file (`geminiService`, `receiptScanner`, `voiceParse`) gọi `supabase.functions.invoke('gemini-proxy')`; `hasRealAi() = key && !IS_MOCK`; thêm mock `functions` vào `MockSupabase`. Set secrets `GEMINI_API_KEY` + `GEMINI_MODEL`; deploy function. **Verify: key KHÔNG còn trong bundle production, round-trip HTTP 200, verify_jwt=True.** Đã update `config.toml`.
- [x] **Blocker #2 — Backup chủ động (pg_dump)**: `.github/workflows/db-backup.yml` (GitHub Actions cron 02:30 UTC hằng ngày + chạy tay) → `pg_dump` toàn DB → gzip → **Artifact `supabase-backup`** (30 ngày); script thủ công `scripts/backup-supabase.sh`. Cần secret `SUPABASE_DB_PASSWORD`. Ghi hướng dẫn backup/restore trong `SCALING_ROADMAP.md`.
- [x] **Blocker #3 — Error tracking (Sentry)**: `@sentry/react` + `src/sentry.js` (bật khi có `VITE_SENTRY_DSN`, dev/mock bỏ qua; tắt session replay để bảo vệ quyền riêng tư); gọi `initSentry()` trong `main.jsx`. Sửa test `daysAgo` dùng giờ địa phương (tránh lệch ±1 ngày do UTC — test bị flaky).
- [x] **Phase 9 — Hạn mức AI + phân gói** (bảo vệ quota free + giới hạn chi phí khi lên paid): `profiles.plan` (free/pro) + bảng `ai_usage` (đếm lượt/ngày) + hàm `get_ai_usage_limit`/`get_ai_usage_today`/`increment_ai_usage` (SECURITY DEFINER) — migration `20260905_phase9_ai_plan.sql` (đã push); `gemini-proxy` (Edge Function) đọc plan + đếm lượt, chặn 429 khi vượt (free=5, pro=100); `callGemini` trả `__quota`; UI Bác sĩ AI hiện "lượt còn lại" + nút "⭐ Nâng cấp Pro"; Admin bật Pro trong RoleManager (nút ⭐ Gói Pro); `fetchAiQuota` tự đọc khi mở tab AI. Secrets: `GEMINI_PROXY_SERVICE_ROLE`. Verify: quota trả về đúng (free, 1/5, còn 4).
- [x] **Export CSV** (lưới an toàn dữ liệu cho nông dân): `csvExport.js` (`toCSV`/`downloadCSV`/`fmtDateCSV` + nhãn) — nút "⬇️ Xuất CSV" ở tab Nhật ký (xuất nhật ký) + card "Tổng hợp sản lượng" (xuất sản lượng); file `.csv` UTF-8 (BOM) mở đúng tiếng Việt trong Excel. Test +7 (79/79). Build + 79/79 test + E2E 2/2 + deploy production HTTP 200.
- [x] **Dọn test account trước public**: xóa 4 tài khoản test (`0999990001`, `0999990002`, `0999990003`, `0999990101` — RLS test + role test) qua GoTrue admin (service_role); xóa kèm dữ liệu vườn/nhật ký/sản lượng (ON DELETE CASCADE). Giữ admin thật `0332643233` (admin_v0, Pro) + demo `0901234567`. Verify còn 2 tài khoản. Back-up thông tin trước khi xóa.
- [x] **Đổi mật khẩu** (không cần mật khẩu cũ — dùng session): `ChangePassword.jsx` trong "Tôi → Đổi mật khẩu"; gọi `supabase.auth.updateUser({password})` dùng session hiện tại (đã verify: không cần mật khẩu cũ); nút "🔒 Xác nhận bằng vân tay/khuôn mặt" (WebAuthn qua `biometric.js`) làm lớp bảo mật bổ sung, graceful fallback nếu thiết bị không hỗ trợ. *(Quên mật khẩu khi đã đăng xuất: cần SMS OTP hoặc admin đặt lại — ghi chú.*)
- [x] **Phase 10 — PoC KB từ video YouTube** (nguồn tri thức cho Bác sĩ AI): Edge Function `kb-from-video` — whitelist `video_sources` (credibility 0–3, khớp bằng `channel_key` ASCII tránh lỗi encoding), chỉ kênh ≥2 mới được; AI tóm tắt/trích cấu trúc; **đối chiếu chéo với KB** (cờ conflict nếu mâu thuẫn hoạt chất); chỉ ghi `raw_articles` **draft** (chưa vào AI), admin duyệt mới published. Migration `20260906_phase10_video_kb.sql` (+ credibility/cross_source/channel_key). Verify: whitelist 4 kênh (Tôi Yêu Hóa Học, Vườn Cây Biết Nói, Happy Farm=2; Vnplant=0 chặn), HTTP 200.
- [x] **Tầng 2 — `kb-ingest` tự động trích raw_articles → kb_entries** (bước quan trọng nhất để KB tiến bộ): Edge Function `kb-ingest` (verify_jwt) đọc raw_articles chưa xử lý → AI trích cấu trúc `kb_entries`; **skip** nếu không phải bệnh/dinh dưỡng cụ thể; `confidence=cao`→published (vào AI), mơ hồ→draft; đánh dấu `status='ingested'`. Migration `20260907_phase11_kb_ingest.sql` (cột `source_raw_id`). **Kết quả: 11 bài thô → 4 bài thành KB (rỉ sắt, xì mủ, đất chua pH, đất chua); KB 13 → 17 bản ghi (7 sầu riêng, 8 cà phê, 2 tiêu).**
- [x] **Tầng 3 — pgvector semantic search** (chuẩn bị KB lớn): extension vector; `kb_entries.embedding vector(3072)` (`gemini-embedding-001`) + hàm `match_kb_similarity` (cosine, published); Edge Function `kb-embed` (backfill/search); `fetchKnowledge` dùng **ngữ nghĩa** trước, fallback lọc theo cây. Migration `20260908_phase12_pgvector.sql`. **Verify: backfill 17/17; search "Cà phê rỉ sắt đốm lá vàng" → rỉ sắt 85%.** *(Lưu ý: pgvector giới hạn index ≤2000 chiều → vector 3072 không tạo index, quét tuần tự — đủ cho KB vài nghìn mục.)*
- [x] **Cron KB tự động**: pg_cron `kb-ingest-daily` (07:00 mỗi ngày) — trích `raw_articles`→`kb_entries` **+ tự tạo embedding inline khi publish**; `kb-crawler-weekly` (06:00 thứ 2). `kb-ingest` đổi sang `verify_jwt=false` + xác thực `x-crawler-secret` (như kb-crawler) để pg_cron gọi được. **KB tự cập nhật mỗi ngày khi có nguồn mới.** Đã verify secret-header gọi 200.
- [x] **Nâng cấp dự báo thời tiết nông vụ (Open-Meteo miễn phí)** — biến dữ liệu thời tiết thành khuyến nghị hành động: `weatherService` thêm chỉ số agri (**độ ẩm đất, nhiệt độ đất, điểm sương, hướng gió, UV, bức xạ, mưa tích lũy, xác suất mưa, dự báo 3 ngày**); `weatherAdvisor.js` (`buildWeatherAdvice`) sinh khuyến nghị theo điều kiện (gió mạnh→ngưng phun, điểm sương thấp→sương giá, mưa lớn→ngưng bón phân, ẩm ấm≥90%→nấm bệnh, đất khô→tưới, UV cao→che nắng); UI hiển thị chỉ số + khuyến nghị + 3 ngày tới. **Không cần mua WeatherPlus/aWhere** vì Open-Meteo free đã có đủ. Test +6 (85/85). Build + 85/85 + E2E 2/2 + deploy HTTP 200.
- [ ] *(Nếu cần)*: domain riêng, OAuth Google/GitHub, tối ưu SEO/meta, thu phí tự động, SMS OTP cho quên mật khẩu, thêm kênh uy tín vào whitelist, nowcasting dông/mưa 1-3h.
...
| 28/08/2026 | DeepSeek Harness | **Production Public + xác minh**: mở khóa site bị "Team protection" (Project visibility=Private → **Public** trong Netlify Visitor access); **xác minh trên `agricommunity.netlify.app`**: đăng nhập `0901234567` ✅, home tải **thời tiết thật** (28°C, mưa phùn, icon SVG mưa, khuyến nghị nông vụ), **badge "VAI TRÒ: NÔNG DÂN"** (phân cấp vai trò), **"Hôm nay vườn cần làm gì?"** + nhật ký (3 hoạt động) → **env vars Supabase hoạt động đúng**. *(Còn lại: thêm domain vào Supabase Auth Allowed Redirect URLs)* |
| 28/08/2026 | DeepSeek Harness | **Cấu hình Supabase Auth redirect** (qua Management API `PATCH /config/auth`): `site_url`=`https://agricommunity.netlify.app`, `uri_allow_list`= `https://agricommunity.netlify.app/**`, `https://agricommunity.netlify.app`; xác minh đọc lại OK (disable_signup=false, mailer_autoconfirm=true). |
| 28/08/2026 | DeepSeek Harness | **Bật CI GitHub Actions (chạy xanh)**: phục hồi `ci.yml` (PAT `workflow`); sửa 3 lỗi — `npm install` thay `npm ci` (EBADPLATFORM esbuild optional-deps), Node 20→22 (vitest4/jsdom30 cần `webidl.util.markAsUncloneable`), E2E cập nhật nút **'Chất cấm'** trong tab **Tôi** + hiệu đề **'Tham khảo chất cấm & hạn chế'**; đồng bộ `package-lock.json` (esbuild@0.28.2). **CI 8/8 bước success** (install + Vitest 68 + build + E2E 2). |
| 28/08/2026 | DeepSeek Harness | **Mốc thời gian nhật ký chuẩn + thời tiết theo vườn**: `daysSince` chuẩn hoá đầu ngày địa phương (tránh lệch UTC); hiển thị "📅 ngày • X ngày trước"; cảnh báo kèm ngày; `formatGardenLogs` thêm "(X ngày trước)" vào prompt AI. **Thời tiết từng vườn**: mỗi vườn lưu vị trí riêng → `fetchGardenWeather` theo tọa độ vườn; card "Hiểu vườn" hiện thời tiết riêng + bấm mở `GardenDetail` (thời tiết khu vực vườn + khu + nhật ký riêng); giữ khung thời tiết toàn cục theo GPS. Build + 69/69 test + E2E 2/2. |
| 28/08/2026 | DeepSeek Harness | **Thu gọn trang chủ (UX gọn)**: "Hôm nay vườn cần làm gì?" thêm công việc theo từng vườn (warn/risk; tất cả ổn → "✅ Các vườn đều ổn"); "Hiểu vườn" thu gọn thành danh sách vườn dạng dòng (tên + thời tiết vườn + trạng thái + nút Xem chi tiết/Ghi nhật ký/Bác sĩ AI) — chi tiết đầy đủ chuyển vào `GardenDetail`. Build + 69/69 test + E2E 2/2. |
| 28/08/2026 | DeepSeek Harness | **Việc cần làm theo vườn + nhắc lịch** (`computeTodayTasks`): tách findings → task (water/fert/spray/issue) với mức ưu tiên (risk→cao) + gợi ý thời điểm; "Hôm nay" mỗi vườn hiện nhiều task; `GardenDetail` card "✅ Việc cần làm hôm nay" (icon + ưu tiên + ⏳). Test +3 (72/72). Build + 72/72 test + E2E 2/2. |
| 28/08/2026 | DeepSeek Harness | **Bỏ "Nhật ký chăm sóc gần đây" + thiết kế lại sản lượng** (Home): loại bỏ card nhật ký gần đây; "Tổng hợp sản lượng thu hoạch" hiện **tổng cả các vườn** (theo cây: "🌳 Sầu riêng (tổng) N kg"…) + **📍 Theo từng vườn** (mỗi vườn: tên + tổng kg + phân theo cây; chưa có dữ liệu → "Chưa có dữ liệu thu hoạch"); cộng đúng theo `yields.garden_id`. Build + 72/72 test + E2E 2/2. |
| 28/08/2026 | DeepSeek Harness | **Soft-delete chống mất dữ liệu + lộ trình mở rộng**: thêm `deleted_at` cho `gardens`/`logs`/`yields`, RLS SELECT `AND deleted_at IS NULL`, partial index, hàm `public.soft_delete`, `GardensManager` đổi `.delete()`→`.update({deleted_at})`; migration `20260904_phase8_soft_delete.sql` đã `db push`. Ghi `docs/SCALING_ROADMAP.md` (bộ nhớ bền vững): không tự xóa data, kế hoạch 0–100/100–1000/>1000 user, chi phí Netlify≈0đ/Supabase/Gemini, cách khôi phục row soft-delete. Build + 72/72 test. |
| 28/08/2026 | DeepSeek Harness | **Blocker #1 — Gemini key sang server** (Edge Function `gemini-proxy`): function giữ `GEMINI_API_KEY` (secret) server-side, `verify_jwt=true`; đổi `geminiService`/`receiptScanner`/`voiceParse` gọi `functions.invoke('gemini-proxy')`; mock `functions` cho `MockSupabase`; set secrets + deploy. **Verify: key không còn trong bundle production, round-trip 200, verify_jwt=True.** Cập nhật `config.toml` + checklist (bỏ blocker #1). Build + 72/72 test + E2E 2/2. |
| 28/08/2026 | DeepSeek Harness | **Blocker #2 — Backup chủ động**: GitHub Actions cron `.github/workflows/db-backup.yml` (pg_dump hằng ngày → Artifact `supabase-backup`, 30 ngày) + script `scripts/backup-supabase.sh`; ghi hướng dẫn backup/restore trong `SCALING_ROADMAP.md` (cần secret `SUPABASE_DB_PASSWORD`). |
| 28/08/2026 | DeepSeek Harness | **Blocker #3 — Error tracking (Sentry)**: `@sentry/react` + `src/sentry.js` (bật khi `VITE_SENTRY_DSN`; mock/dev bỏ qua; tắt session replay); `initSentry()` trong `main.jsx`; sửa test `daysAgo` dùng giờ địa phương (fix flaky ±1 ngày). Build + 72/72 test. |
| 28/08/2026 | DeepSeek Harness | **Phase 9 — Hạn mức AI + phân gói**: `profiles.plan` (free/pro) + `ai_usage` (đếm lượt/ngày) + hàm SECURITY DEFINER; `gemini-proxy` chặn vượt hạn mức (free=5/pro=100), trả `__quota`; UI Bác sĩ AI hiện "lượt còn lại" + nút nâng cấp; Admin bật Pro trong RoleManager; `fetchAiQuota` khi mở tab AI. Secret `GEMINI_PROXY_SERVICE_ROLE`. Verify quota (free, 1/5, còn 4). Build + 72/72 test + E2E 2/2 + deploy production HTTP 200. |
| 28/08/2026 | DeepSeek Harness | **Export CSV** (nhật ký + sản lượng): `csvExport.js` (toCSV/downloadCSV/fmtDateCSV + nhãn tiếng Việt) + nút "⬇️ Xuất CSV" ở tab Nhật ký & card sản lượng; file `.csv` UTF-8 (BOM) mở đúng tiếng Việt trong Excel; lưới an toàn dữ liệu cho nông dân. Test +7 (79/79). Build + 79/79 test + E2E 2/2 + deploy HTTP 200. |
| 28/08/2026 | DeepSeek Harness | **Dọn test account trước public**: xóa 4 tài khoản test (`0999990001`/`0999990002`/`0999990003`/`0999990101`) qua GoTrue admin (service_role) + dữ liệu kèm; giữ admin thật `0332643233` (admin_v0, Pro) + demo `0901234567`. Verify còn 2 tài khoản. |
| 28/08/2026 | DeepSeek Harness | **Đổi mật khẩu** (không cần mật khẩu cũ — dùng session): `ChangePassword.jsx` (Tôi → Đổi mật khẩu) gọi `auth.updateUser({password})` bằng session; nút "🔒 Xác nhận vân tay/khuôn mặt" (WebAuthn `biometric.js`) lớp bảo mật bổ sung, graceful fallback. *(Quên mật khẩu khi đã đăng xuất: cần SMS OTP/admin — ghi chú.)* Build + 79/79 test + E2E 2/2 + deploy HTTP 200. |
| 28/08/2026 | DeepSeek Harness | **Phase 10 — PoC KB từ video YouTube**: Edge Function `kb-from-video` (whitelist `video_sources` credibility≥2, khớp `channel_key` ASCII, AI tóm tắt, đối chiếu chéo KB, chỉ ghi `raw_articles` draft). Migration `20260906_phase10_video_kb.sql`. Verify HTTP 200, draft_id=10, credibility=2, conflict=false. |
| 28/08/2026 | DeepSeek Harness | **Tầng 2 — `kb-ingest` tự động trích KB**: Edge Function đọc raw_articles → AI trích cấu trúc `kb_entries`, skip nếu không phải bệnh/dinh dưỡng, published (cao)/draft (mơ hồ), đánh dấu ingested. Migration `20260907_phase11_kb_ingest.sql`. Kết quả: 11 bài thô → 4 bài thành KB (rỉ sắt, xì mủ, đất chua pH, đất chua); KB 13→17 (7 sầu riêng, 8 cà phê, 2 tiêu). |
| 28/08/2026 | DeepSeek Harness | **Tầng 3 — pgvector semantic search**: extension vector + `kb_entries.embedding vector(3072)` (gemini-embedding-001) + `match_kb_similarity`; Edge Function `kb-embed` (backfill/search); `fetchKnowledge` dùng ngữ nghĩa trước, fallback lọc cây. Migration `20260908_phase12_pgvector.sql`. Verify: backfill 17/17; search rỉ sắt → 85%. *(pgvector index ≤2000 chiều → không index, quét tuần tự.)* |
| 28/08/2026 | DeepSeek Harness | **Cron KB tự động**: `kb-ingest-daily` (07:00 pg_cron) trích raw_articles→kb_entries + tạo embedding inline khi publish; `kb-crawler-weekly` (06:00 thứ 2). `kb-ingest` → `verify_jwt=false` + xác thực `x-crawler-secret`. KB tự cập nhật mỗi ngày. Verify secret-header gọi 200. |
| 28/08/2026 | DeepSeek Harness | **Nâng cấp thời tiết nông vụ (Open-Meteo free)**: `weatherService` thêm chỉ số agri (độ ẩm đất, nhiệt độ đất, điểm sương, hướng gió, UV, mưa tích lũy, xác suất mưa, dự báo 3 ngày); `weatherAdvisor.js` sinh khuyến nghị hành động (ngưng phun khi gió mạnh, sương giá, ngưng bón phân khi mưa lớn, nấm bệnh khi ẩm ấm, tưới khi đất khô, che nắng UV cao); UI hiển thị chỉ số + khuyến nghị + 3 ngày. Không cần mua API trả phí. Test +6 (85/85). Build + 85/85 + E2E 2/2 + deploy HTTP 200. |
| 28/08/2026 | DeepSeek Harness | **Thiết kế lại UI Bác sĩ cây trồng (chat-first)**: header gọn (title + toggle Vườn tôi/Vườn khác + Mới), hạn mức thành dòng nhỏ, chat rộng **46–58vh**, "Gắn vườn/khu" + "Thử nghiệm nhanh" **thu gọn (collapsible)**, input to 48px. Kết nối `weatherAdvisor` vào "Hôm nay vườn cần làm gì?" + nút "Kiểm tra bằng Bác sĩ AI". **Fix**: bỏ nút "Chụp ảnh lá cây"/"Xóa hết" **trùng lặp (2→1)**, sửa block dồn cột (thêm `.camera-box--inline`), gộp hướng dẫn 1 dòng. Deploy production (token Netlify mới). Verify production: 1 nút chụp ảnh, không lỗi. |
| 28/08/2026 | DeepSeek Harness | **Token Netlify cũ hết hạn (401)** → không deploy được; đã xin token mới và deploy thành công. Ghi chú: Netlify không có webhook GitHub → deploy thủ công qua CLI (`netlify-cli deploy --dir dist --prod`). |
