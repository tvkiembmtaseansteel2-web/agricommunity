...
- [ ] *(Còn lại — cần bạn thao tác trên dashboard/CLI)*: Thêm **domain của bạn** vào **Allowed Redirect URLs** trong Supabase Auth (để đăng nhập hoạt động trên production); cung cấp `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `VITE_GEMINI_API_KEY` trong Netlify/Vercel dashboard; PWA cần HTTPS (production tự có).
