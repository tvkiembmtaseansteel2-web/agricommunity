...
- [ ] *(Còn lại — cần bạn thao tác)*: **Push lên GitHub** (`git remote add origin ... && git push -u origin main`) → **Import vào Netlify** (đã tự nhận `netlify.toml`) → đặt biến môi trường trên Netlify (`VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `VITE_GEMINI_API_KEY`) → Redeploy → thêm domain vào **Allowed Redirect URLs** trong Supabase Auth.
